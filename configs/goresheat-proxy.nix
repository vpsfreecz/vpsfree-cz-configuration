...
let
  ...
  allNodes = filter (
    m: m.metaConfig.node != null && m.metaConfig.monitoring.enable && isNull m.carrier
  ) allMachines;

  upstreamName =
    node: "goresheat_${builtins.replaceStrings [ "." ] [ "_" ] node.metaConfig.host.fqdn}";

  mkUpstreams = listToAttrs (
    map (
      m:
      nameValuePair (upstreamName m) {
        servers = {
  ...
  mkGoresheatWebsockets = listToAttrs (
    map (
      m:
      nameValuePair "/${m.metaConfig.host.fqdn}/ws" {
        priority = 400;
  ...
  mkGoresheatRoots = listToAttrs (
    map (
      m:
      nameValuePair "/${m.metaConfig.host.fqdn}/" {
        priority = 500;
  ...
  indexFile = pkgs.writeText "goresheat-index.html" ''
    ...
    <html>
      ...
      <body>
        ...
        <ul>
          ${concatMapStringsSep "\n" (
            m: ''<li><a href="/${m.metaConfig.host.fqdn}/">${m.metaConfig.host.fqdn}</a></li>''
          ) allNodes}
        </ul>

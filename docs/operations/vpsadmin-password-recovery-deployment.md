...
```shell
VPSADMIN_REVISION=050ea5263812a76dc39f14c5b58a0e88714d634d
...
Pending MFA and required-password-change login tokens issued by the preceding
API do not carry an authentication generation. The upgraded API rejects these
unfinished logins, so those users must sign in again. Established sessions
remain usable. Explicitly stamped tokens remain subject to the normal
generation check.

User Create now enforces the documented password minimum for an explicitly
supplied initial password. Provisioning scripts that supplied shorter values
must use passwords meeting that requirement. Omitting the optional password
continues to create an account with password login disabled. Existing account

...
checks apply only to the compatibility cleanup described here; they are not
...
reporting Node and supervisor. Its seven core migrations must already be up,
and every eligible Node/storage record must have complete kernel and system
history backfill checkpoints. Do not repeat those backfills for this release
or use this runbook until that premise has been verified.
...
```shell
VPSADMIN_REVISION=88f03da4455f4d709ca64785b1db14db834f323a
```
